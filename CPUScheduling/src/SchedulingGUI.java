...
public class SchedulingGUI extends JFrame {
    ...
    private void initComponents() {
        ...
        jLabel4.setText("OS Scheduler");
        jPanel1.add(jLabel4, new AbsoluteConstraints(320, 10, 420, 80));
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) throws FileNotFoundException, IOException, NoSuchAlgorithmException {
        String desktop_path = "";
        String operating_system = System.getProperty("os.name");

            if(operating_system.startsWith("Windows")) {
                desktop_path = System.getenv("USERPROFILE") + "\\Desktop\\";
            } else if (operating_system.startsWith("Mac")) {
                desktop_path = System.getProperty("user.home") + "/Desktop/";
            } else {
                desktop_path = System.getProperty("user.home") + "/Desktop/";
            }

        String folder_name = "OS Scheduler Finished Files";
        String folder_path = desktop_path + folder_name;
        File folder = new File(folder_path);

        if (!folder.exists()) {
            if (folder.mkdir()) {
                JOptionPane.showMessageDialog(null, "OS Scheduler is ready to launch\n" + "Launch now!");
            } else {
                JOptionPane.showMessageDialog(null, "OS Scheduler is not ready to launch!\n" + "Launch failed!");
            }
        } else {
            JOptionPane.showMessageDialog(null, "OS Scheduler is ready to launch\n" + "Launch now!");
        }
        ...
        File copiedFile = new File(folder, file.getName());
        sourcePath = file.getAbsolutePath();

        String[] expectedExtension = {".bat", ".rar", ".txt", 
                                            ".zip", ".dmg",
                                            ".ods", ".csv"};

        boolean isExtensionMatching = false;

            for (String extension : expectedExtension) {
                if (file_name.endsWith(extension)) {
                    isExtensionMatching = true;
                    break;
                }
            }

                if (isExtensionMatching) {
                    JOptionPane.showMessageDialog(null, "This file might be corrupted or containing a virus!", "Corrupted File Detector", JOptionPane.OK_OPTION);
                    setVisible(false);
                    SchedulingGUI schedulingGUI = new SchedulingGUI();
                    schedulingGUI.setVisible(true);
                } else {
                    JOptionPane.showMessageDialog(null, "This file is not corrupted or containing a virus!");
                    jButton1.setText(file_name);
                    if (copiedFile.exists()) {
                        JOptionPane.showMessageDialog(null, "Start Processing", "OS Scheduler Processing", JOptionPane.OK_OPTION);
                    } else {
                        try {
                            Files.copy(file.toPath(), copiedFile.toPath());
                            JOptionPane.showMessageDialog(null, "Start Processing", "OS Scheduler Processing", JOptionPane.INFORMATION_MESSAGE);
                        } catch (IOException e) {
                            JOptionPane.showMessageDialog(null, "An error occurred while start processing!", "OS Scheduler Processing Error", JOptionPane.ERROR_MESSAGE);
                        }
                    }
                    // Files.copy(Paths.get(source_of_file), Paths.get(destinationFile), StandardCopyOption.REPLACE_EXISTING);
